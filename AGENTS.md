...
**harness-dashboard** — a local-first dashboard for Codex token usage, costs, and session
...
Codex writes one JSONL file per session to `~/.Codex/projects/<slug>/<session>.jsonl`.
...
- **Streaming-snapshot dedup.** The dedup key is `(session_id, message_id)`, not the per-line
  `uuid`. Codex writes multiple snapshot lines per assistant response sharing one
